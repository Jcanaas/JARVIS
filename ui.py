...
class C:
    # Paleta "Aether AI" (Stitch / NEURAL OS): glassmorphism oscuro sobre
    # degradado azul profundo, primario cornflower #5E82FF y tono claro #B6C4FF.
    BG        = "#0A0C16"
    PANEL     = "#141829"
    PANEL2    = "#1E2338"
    BORDER    = "#3A4160"
    ...
    def __init__(self, parent=None):
        ...
        self.view().setStyleSheet(f"""
            QListView {{
                ...
                padding: 6px;
                outline: none;
                selection-background-color: rgba(182, 196, 255, 0.24);
...
def _wa_settings_icon(name: str, color: str, size: int = 18) -> QIcon:
    """Stroke-drawn line icon for the rule cards, matching the app's icon style."""
    pm = QPixmap(size, size)
    pm.fill(Qt.GlobalColor.transparent)
    p = QPainter(pm)
    p.setRenderHint(QPainter.RenderHint.Antialiasing)
    ...
    if name == "up":
        path = QPainterPath(); path.moveTo(6, 14.5); path.lineTo(12, 8.5); path.lineTo(18, 14.5)
        p.drawPath(path)
    elif name == "down":
        path = QPainterPath(); path.moveTo(6, 9.5); path.lineTo(12, 15.5); path.lineTo(18, 9.5)
        p.drawPath(path)
    elif name == "edit":
        path = QPainterPath()
        path.moveTo(4, 20); path.lineTo(4, 16); path.lineTo(14.5, 5.5)
        path.lineTo(18.5, 9.5); path.lineTo(8, 20); path.closeSubpath()
        p.drawPath(path)
        line(12.5, 7.5, 16.5, 11.5)
    elif name == "trash":
        line(4, 6.5, 20, 6.5)
        handle = QPainterPath()
        handle.moveTo(9.5, 6.5); handle.lineTo(9.5, 4.2); handle.lineTo(14.5, 4.2); handle.lineTo(14.5, 6.5)
        p.drawPath(handle)
        body = QPainterPath()
        body.moveTo(6, 6.5); body.lineTo(7, 20); body.lineTo(17, 20); body.lineTo(18, 6.5)
        p.drawPath(body)
...
class WhatsAppRuleDialog(QDialog):
    ...
    def __init__(self, parent=None, rule: dict | None = None):
        ...
        for i, label in enumerate(SettingsModePanel._WA_DAYS):
            b = QPushButton(label)
            b.setCheckable(True)
